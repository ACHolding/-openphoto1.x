#!/usr/bin/env python3
# openphoto_1.0.py
# OpenPhoto 1.0 — DiffusionBee-style desktop app (single file — no extra apps).
...
APP_TITLE = "OpenPhoto 1.0"
APP_NAME = "OpenPhoto 1.0"
TITLEBAR_H = 55
SIDEBAR_W = 200
...
LIGHT_THEME = {
    ...
    "SIDEBAR_SEL": "#E6E6E6",
    "INPUT_BG": "#F1F4FA",
    "CONTENT": "#F2F2F2",  # DiffusionBee .animatable_content_box
    "ENGINE": "#E8E8E8",  # DiffusionBee .image_area (~rgba(0,0,0,0.05))
    "ENGINE_BORDER": "#00000000",
    "OK": "#28c840",
    "WARN": "#c77c00",
    "ERR": "#ff5f57",
}
DARK_THEME = {
    ...
    "SIDEBAR_SEL": "#1a1b1f",
    "INPUT_BG": "#111318",
    "CONTENT": "#1c1c1c",  # DiffusionBee dark .animatable_content_box
    "ENGINE": "#302e2f",  # DiffusionBee dark .image_area
    "ENGINE_BORDER": "#606060",
    "OK": "#28c840",
    "WARN": "#febc2e",
    "ERR": "#ff5f57",
}
...
ENGINE = LIGHT_THEME["ENGINE"]
ENGINE_BORDER = LIGHT_THEME["ENGINE_BORDER"]
OK = LIGHT_THEME["OK"]
...
def apply_theme(dark: bool = False) -> None:
    ...
    global ENGINE, ENGINE_BORDER, OK, WARN, ERR, DARK_MODE
    ...
    ENGINE_BORDER = palette["ENGINE_BORDER"]
    OK = palette["OK"]
    WARN = palette["WARN"]
    ERR = palette["ERR"]
...
# macOS 27 / Tk 9: [tk scaling] can be NaN and abort Tk init (segfault / TclError).
_SCALING_PCT_PATCH = r'''proc ::tk::ScalingPct {} {
    set s [tk scaling]
    if {![string is double -strict $s] || $s != $s || $s <= 0} {
...
def _discover_stock_tk_library() -> Optional[Path]:
    """Find the interpreter's shipped tk*.0 library (uv/CPython layout)."""
    bases = [Path(_sys.base_prefix), Path(_sys.prefix)]
    try:
        import _tkinter as _tki  # type: ignore

        so = Path(getattr(_tki, "__file__", "") or "")
        if so.is_file():
            # .../lib/python3.11/lib-dynload/_tkinter.*.so -> install root
            bases.append(so.resolve().parents[3])
            bases.append(so.resolve().parents[2])  # .../lib (tk9.0 sibling)
    except Exception:
        pass
    seen: set[Path] = set()
    for base in bases:
        for candidate in (
            ...
            if candidate in seen:
                continue
            seen.add(candidate)
            if (candidate / "tk.tcl").is_file() and (candidate / "scaling.tcl").is_file():
                return candidate
    return None


def prepare_macos_tk() -> None:
    """Install a cached Tk library with NaN-safe scaling for macOS pre-release."""
    if _sys.platform != "darwin":
        return
    if os.environ.get("OPENPHOTO_SKIP_TK_FIX"):
        return
    stock = _discover_stock_tk_library()
    if stock is None:
        return
    fixed = HOME / f"{stock.name}-macos-fix"
    marker = fixed / ".openphoto-tk-fix-v2"
    try:
        if not marker.is_file():
            HOME.mkdir(parents=True, exist_ok=True)
            if fixed.exists():
                shutil.rmtree(fixed)
            shutil.copytree(stock, fixed)
            scaling = fixed / "scaling.tcl"
            text = scaling.read_text(encoding="utf-8")
            start = text.find("proc ::tk::ScalingPct {}")
            end = text.find("\nproc ::tk::ScalingInitX11", start)
            patched = False
            if start >= 0 and end > start:
                scaling.write_text(text[:start] + _SCALING_PCT_PATCH + text[end + 1 :], encoding="utf-8")
                patched = True
            # Force a sane scale before icons.tcl evaluates ScalingPct.
            tk_tcl = fixed / "tk.tcl"
            body = tk_tcl.read_text(encoding="utf-8")
            needle = "namespace eval ::tk {\n\tSourceLibFile icons"
            insert = "namespace eval ::tk {\n\tcatch {tk scaling 2.0}\n\tSourceLibFile icons"
            if needle in body:
                tk_tcl.write_text(body.replace(needle, insert, 1), encoding="utf-8")
                patched = True
            if not patched:
                shutil.rmtree(fixed, ignore_errors=True)
                return
            marker.write_text("1\n", encoding="utf-8")
        os.environ["TK_LIBRARY"] = str(fixed)
        tcl_stock = stock.parent / ("tcl9.0" if stock.name.startswith("tk9") else "tcl8.6")
        if tcl_stock.is_dir() and "TCL_LIBRARY" not in os.environ:
            os.environ["TCL_LIBRARY"] = str(tcl_stock)
    except OSError:
        # Leave stock Tk in place; create may still fail with a clear TclError.
        pass


def _sanitize_tk_scaling(root: tk.Misc) -> None:
    try:
        raw = float(root.tk.call("tk", "scaling"))
...
def diffusionbee_image_stem(prompt: str) -> str:
    """Match DiffusionBee latest files: first 30 alphanumeric prompt chars + random int."""
    slug = "".join(ch for ch in str(prompt or "")[:30] if ch.isalnum()) or "image"
    return f"{slug}_{random.randint(0, 100000000)}"
...
def is_video_bytes(data: bytes) -> bool:
    ...
    if b"ftyp" in data[:64]:
        return True
    if data[:4] == b"RIFF" and data[8:12] in (b"AVI ", b"AVIX"):
        return True
    return False


def ensure_png_bytes(data: bytes) -> bytes:
    """JPEG/WebP off. Convert any still image to PNG like DiffusionBee."""
    if not data:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        converters = []
        if _sys.platform == "darwin":
            converters.append(["sips", "-s", "format", "png", str(src), "--out", str(dst)])
        converters.append(["convert", str(src), str(dst)])
        for cmd in converters:
            ...
            if proc.returncode == 0 and dst.is_file():
                ...
                if out.startswith(PNG_MAGIC):
                    return out
    raise RuntimeError("Could not convert result to PNG (jpeg output is off)")


def dimensions_from_b64(b64: str) -> tuple[int, int]:
    """Read PNG/JPEG dimensions from a base64 payload (no PIL)."""
    import struct
    try:
        raw = base64.b64decode(b64)
    except Exception:
        return 1024, 1024
    if raw.startswith(PNG_MAGIC) and len(raw) >= 24:
...
def snap_dim(n: int, max_dim: int = 2048) -> int:
    return min(max(256, (int(n) // 16) * 16), max_dim)


def apply_negative_prompt(prompt: str, negative: str) -> str:
    """DiffusionBee-style negative prompt (FLUX has no native field)."""
...
def load_preview_photo(path: str, max_w: int = 420, max_h: int = 520) -> Optional[tk.PhotoImage]:
    """Instant preview — PhotoImage first; optional sips only if Tk can't open the file."""
    p = Path(path)
    if not p.is_file():
        return None
    try:
    ...
    if _sys.platform == "darwin":
        try:
            with tempfile.TemporaryDirectory() as td:
                thumb = Path(td) / "thumb.png"
                subprocess.run(
                    ["sips", "-Z", str(max(max_w, max_h)), str(p), "--out", str(thumb)],
...
# DiffusionBee applet defaults per tool mode
MODE_DEFAULT_MODEL: dict[str, str] = {
    "txt2img": DEFAULT_FLUX_MODEL,
    "img2img": "FLUX.1 Kontext [pro]",
    "inpaint": "FLUX.1 Fill [pro]",
    "upscale": "FLUX.1 Kontext [max]",
    "outpaint": "FLUX.1 Kontext [pro]",
}
...
def is_model_separator(label: str) -> bool:
    s = str(label or "").strip()
    return s.startswith("——") or s.startswith("---")


def flux_models_by_family(kind: str = "image", img2img_only: bool = False) -> list[str]:
    """DiffusionBee-style grouped picker: all FLUX photo/video versions by family."""
    out: list[str] = []
    for fam in FAMILY_ORDER:
        names = [
            k for k, v in FLUX_MODELS.items()
            if v.get("family") == fam and v.get("kind") == kind
            and (not img2img_only or v.get("img2img") or v.get("fill_mask"))
        ]
        if not names:
            continue
        title = FAMILY_TITLES.get(fam, fam)
        out.append(f"{SEPARATOR_PREFIX}{title} ——")
        out.extend(names)
    known = set(out)
    extra = [
        k for k, v in FLUX_MODELS.items()
        if v.get("kind") == kind and k not in known and not is_model_separator(k)
        and (not img2img_only or v.get("img2img") or v.get("fill_mask"))
    ]
    if extra:
        out.append(f"{SEPARATOR_PREFIX}Other ——")
        out.extend(extra)
    return out


def resolve_flux_model_choice(choice: str, kind: str = "image") -> str:
    if choice and not is_model_separator(choice) and choice in FLUX_MODELS:
        return choice
    names = [k for k, v in FLUX_MODELS.items() if v.get("kind") == kind]
    return names[0] if names else DEFAULT_FLUX_MODEL
...
HOME_SECTIONS = [
    (
        ...
        [
            ...
            ("Outpainting", "Extend an image beyond its borders", "canvas", "outpaint"),
...
PAGE_TABS = {"home", "models", "history", "settings", "training", "llms"}
...
class AppConfig:
    ...
    output_format: str = "png"  # jpeg off — DiffusionBee always writes PNG
    ...
    def load(cls) -> "AppConfig":
        ...
        if CONFIG_PATH.exists():
            try:
                ...
                pass
        cfg.output_format = "png"  # jpeg = off for every Flux model
...
class HistoryStore:
    ...
    def load(self) -> None:
        with self._lock:
            ...
            if not isinstance(raw, list):
                self.items = []
                return
            fields_ok = {f.name for f in fields(HistoryItem)}
            items: list[HistoryItem] = []
            for row in raw:
                if not isinstance(row, dict):
                    continue
                try:
                    cleaned = {k: v for k, v in row.items() if k in fields_ok}
                    items.append(HistoryItem(**cleaned))
                except Exception:
                    continue
            self.items = items
    ...
    def save(self) -> None:
        with self._lock:
            HOME.mkdir(parents=True, exist_ok=True)
            payload = json.dumps([asdict(i) for i in self.items], indent=2)
    ...
    def snapshot(self, limit: int = 200) -> list[HistoryItem]:
        with self._lock:
...
class FluxBackend:
    ...
    def generate(
        ...
        if not self.api_key:
            ...
            needs_input = bool(input_image or mask_image or start_video or input_images)
            if needs_input:
                raise RuntimeError(
                    "Image / mask / video inputs need a BFL API key. "
                    "Free mode only supports text → image."
                )
            if kind == "video" and video_mode in ("i2v", "v2v"):
                raise RuntimeError(
                    "Image/video-to-video needs a BFL API key. "
                    "Free mode only makes a cinematic still from text."
                )
        ...
        if on_status:
            on_status(f"Submitted ({str(request_id)[:8]}…) — waiting")

        timeout = 900 if kind == "video" else 300
        poll_every = 0.25 if kind == "video" else 0.15
    ...
    def _generate_free(
        ...
        params = (
            ...
            f"&seed={int(seed)}&nologo=true&enhance=false&safe=false&format=png"
        )
        url = FREE_IMAGE_API.format(prompt=q) + "?" + params
        if on_status:
            on_status("Fetching image…")

        req = urllib.request.Request(
            url,
            headers={"User-Agent": "OpenPhoto/0.1", "Accept": "image/png"},
        )
        try:
    ...
    def _image_payload(
        ...
        payload["output_format"] = "png"  # jpeg off — all Flux image models
    ...
    def save_result(
        ...
        if kind == "video" or is_video_bytes(data):
            ext = ".mp4"
            if data[:4] == b"RIFF" and data[8:12] in (b"AVI ", b"AVIX"):
                ext = ".avi"
...
class LLMBackend:
    ...
    def _local_vibe_enhance(self, prompt: str, mode: str, vibe: str) -> str:
        ...
        if mode in ("img2img", "inpaint", "canvas", "outpaint"):
...
class DiffusionEngine:
    ...
    def off_job(self, cb: Callable[[GenJob], None]) -> None:
    ...
    def enqueue(self, **kwargs) -> GenJob:
        job_id = kwargs.pop("job_id", None) or uuid.uuid4().hex[:12]
        job = GenJob(id=job_id, **kwargs)
    ...
    def _run(self, job: GenJob) -> None:
        ...
        full_prompt = apply_negative_prompt(prompt, job.negative_prompt)
        width, height = job.width, job.height
        input_image = job.input_image
        mask_image = job.mask_image

        if job.mode == "upscale" and input_image:
            iw, ih = dimensions_from_b64(input_image)
            factor = max(2, min(int(job.upscale_factor or 2), 4))
            width = snap_dim(iw * factor)
            height = snap_dim(ih * factor)
            if "upscale" not in full_prompt.lower():
                full_prompt = (
                    f"{full_prompt}. High resolution AI upscale, sharp fine detail, "
                    "clean textures, no blur."
                )
        elif job.mode == "outpaint" and input_image:
            iw, ih = dimensions_from_b64(input_image)
            width = snap_dim(int(iw * 1.35))
            height = snap_dim(int(ih * 1.35))
            if "extend" not in full_prompt.lower():
                full_prompt = (
                    f"{full_prompt}. Seamlessly extend the scene beyond the original "
                    "borders, matching style and lighting."
                )
        elif job.mode == "inpaint" and input_image and not mask_image:
            if "inpaint" not in full_prompt.lower() and "fill" not in full_prompt.lower():
                full_prompt = f"{full_prompt}. Inpaint only the masked region naturally."

        steps_val = job.steps_override if job.steps_override is not None else self.cfg.steps
        guidance_val = job.guidance_override if job.guidance_override is not None else self.cfg.guidance
        ...
        result = self.flux.generate(
            ...
            steps=steps_val if use_flex else None,
            guidance=guidance_val if use_flex else None,
        ...
        stem = f"{prefix}_{job.id}" if kind == "video" else diffusionbee_image_stem(full_prompt)
        ...
        self._emit(job)  # paint preview immediately
        try:
            self.history.add(
                HistoryItem(
                    id=job.id,
...
class LLMDownloadWindow(tk.Toplevel):
    ...
    def _list_repo(self):
        ...
        def work():
            try:
                self.app.schedule_ui(lambda: self.status.configure(text="Listing repo…", fg=WARN) if self.winfo_exists() else None)
                ...
                self.app.schedule_ui(done)
            except Exception as e:
                err = str(e)
                self.app.schedule_ui(lambda: self.status.configure(text=err, fg=ERR) if self.winfo_exists() else None)
        ...
        def work():
            try:
                ...
                def fail():
                    ...
                    self._busy = False
                    self.dl_btn.configure(state="normal")
                    self.status.configure(text=err, fg=ERR)
                    messagebox.showerror(APP_NAME, f"Download failed:\n{err}")
                self.app.schedule_ui(fail)
...
class ToolWorkspace(tk.Toplevel):
    def __init__(self, app: "App", title: str, mode: str):
        ...
        tk.Label(
            head,
            text="DiffusionBee-style · local FLUX engine · offline when keyed",
        ...
        # DiffusionBee TwoColAppletLayout: left_controls max-width 350 + thin border
        left = tk.Frame(
            body, bg=CONTENT, width=350,
        ...
        self.prompt.pack(fill="both", expand=True, pady=(6, 8))
        self.prompt.insert("1.0", "Describe what you want to create or edit…")

        tk.Label(left, text="Negative prompt", bg=CONTENT, fg=MUTED, font=(UI_FONT, 12)).pack(anchor="w")
        self.negative = tk.Text(
            left, bg=INPUT_BG, fg=TEXT, insertbackground=TEXT,
            relief="solid", borderwidth=1, highlightthickness=0,
            font=(UI_FONT, 12), height=3, wrap="word",
        )
        self.negative.pack(fill="x", pady=(6, 10))
        if app.cfg.negative_prompt:
            self.negative.insert("1.0", app.cfg.negative_prompt)

        opts = tk.Frame(left, bg=CONTENT)
        opts.pack(fill="x", pady=(0, 10))

        model_label = "FLUX 3 video model" if self.is_video else "FLUX photo model (all versions)"
        tk.Label(opts, text=model_label, bg=CONTENT, fg=MUTED, font=(UI_FONT, 11)).grid(row=0, column=0, sticky="w")
        img2img_only = mode in ("img2img", "inpaint", "canvas", "upscale", "outpaint")
        model_values = flux_models_by_family(
            "video" if self.is_video else "image",
            img2img_only=img2img_only and not self.is_video,
        )
        default_model = MODE_DEFAULT_MODEL.get(mode) or (
            app.cfg.video_model if self.is_video else app.cfg.flux_model
        )
        flat = [m for m in model_values if not is_model_separator(m)]
        if default_model not in flat and flat:
            default_model = flat[0]
        self.model_var = tk.StringVar(value=default_model)
        self._last_real_model = default_model
        self.model_menu = ttk.Combobox(
            opts, textvariable=self.model_var, values=model_values,
            state="readonly", width=36,
        )
        self.model_menu.grid(row=1, column=0, sticky="w", padx=(0, 14))
        self.model_menu.bind("<<ComboboxSelected>>", self._on_model_picked)

        if self.is_video:
            tk.Label(opts, text="Mode", bg=CONTENT, fg=MUTED, font=(UI_FONT, 11)).grid(row=0, column=1, sticky="w")
            ...
            tk.Label(opts, text="Resolution", bg=CONTENT, fg=MUTED, font=(UI_FONT, 11)).grid(row=0, column=2, sticky="w")
            ...
            tk.Label(opts, text="Seconds", bg=CONTENT, fg=MUTED, font=(UI_FONT, 11)).grid(row=2, column=0, sticky="w", pady=(10, 0))
            ...
            tk.Checkbutton(
                ...
                bg=CONTENT, fg=TEXT, selectcolor=PANEL, activebackground=CONTENT, activeforeground=TEXT,
                font=(UI_FONT, 11),
            ).grid(row=3, column=1, sticky="w")
            self.size_var = tk.StringVar(value="")
        else:
            tk.Label(opts, text="Size", bg=CONTENT, fg=MUTED, font=(UI_FONT, 11)).grid(row=0, column=1, sticky="w")
            ...
            if mode == "upscale":
                tk.Label(opts, text="Scale", bg=CONTENT, fg=MUTED, font=(UI_FONT, 11)).grid(
                    row=2, column=0, sticky="w", pady=(10, 0),
                )
                factor_default = f"{app.cfg.upscale_factor}×" if app.cfg.upscale_factor in (2, 4) else "2×"
                self.upscale_var = tk.StringVar(value=factor_default)
                ttk.Combobox(
                    opts, textvariable=self.upscale_var, values=UPSCALE_FACTORS,
                    state="readonly", width=10,
                ).grid(row=3, column=0, sticky="w", padx=(0, 14))
            else:
                self.upscale_var = tk.StringVar(value="2×")

        seed_lab_row = 2 if self.is_video else 0
        seed_ent_row = 3 if self.is_video else 1
        tk.Label(opts, text="Seed (-1 random)", bg=CONTENT, fg=MUTED, font=(UI_FONT, 11)).grid(
            row=seed_lab_row, column=2, sticky="w",
        ...
        tk.Entry(opts, textvariable=self.seed_var, bg=INPUT_BG, fg=TEXT, insertbackground=TEXT,
                 relief="flat", width=12).grid(row=seed_ent_row, column=2, sticky="w")

        meta0 = flux_meta(default_model)
        self.model_hint = tk.Label(
            opts,
            text=meta0.get("label") or FAMILY_TITLES.get(meta0.get("family", ""), ""),
            bg=CONTENT, fg=MUTED, font=(UI_FONT, 10),
        )
        hint_row = 4 if self.is_video else (4 if mode == "upscale" else 2)
        self.model_hint.grid(row=hint_row, column=0, columnspan=3, sticky="w", pady=(8, 0))

        self.enhance_var = tk.BooleanVar(value=app.cfg.enhance_prompts and mode not in ("upscale",))
        tk.Checkbutton(
            left, text="Enhance prompt with LLM before generate",
            variable=self.enhance_var, bg=CONTENT, fg=TEXT, selectcolor=PANEL,
            activebackground=CONTENT, activeforeground=TEXT, font=(UI_FONT, 12),
        ).pack(anchor="w", pady=(4, 4))

        # DiffusionBee Advanced options (steps / CFG / batch)
        adv_head = tk.Frame(left, bg=CONTENT, cursor="hand2")
        adv_head.pack(fill="x", pady=(2, 0))
        self._adv_head = adv_head
        self._adv_open = tk.BooleanVar(value=False)
        self._adv_toggle_lbl = tk.Label(
            adv_head, text="▸ Advanced options", bg=CONTENT, fg=BLUE,
            font=(UI_FONT, 12, "bold"), cursor="hand2",
        )
        self._adv_toggle_lbl.pack(anchor="w")
        self._adv_toggle_lbl.bind("<Button-1>", lambda e: self._toggle_advanced())
        adv_head.bind("<Button-1>", lambda e: self._toggle_advanced())
        self._adv_frame = tk.Frame(left, bg=CONTENT)
        adv = self._adv_frame
        ar = tk.Frame(adv, bg=CONTENT)
        ar.pack(fill="x", pady=(6, 0))
        tk.Label(ar, text="Steps", bg=CONTENT, fg=MUTED, font=(UI_FONT, 11)).grid(row=0, column=0, sticky="w")
        self.steps_var = tk.StringVar(value=str(app.cfg.steps))
        tk.Entry(ar, textvariable=self.steps_var, bg=INPUT_BG, fg=TEXT, insertbackground=TEXT,
                 relief="flat", width=8).grid(row=1, column=0, sticky="w", padx=(0, 12))
        tk.Label(ar, text="CFG / guidance", bg=CONTENT, fg=MUTED, font=(UI_FONT, 11)).grid(row=0, column=1, sticky="w")
        self.guidance_var = tk.StringVar(value=str(app.cfg.guidance))
        tk.Entry(ar, textvariable=self.guidance_var, bg=INPUT_BG, fg=TEXT, insertbackground=TEXT,
                 relief="flat", width=8).grid(row=1, column=1, sticky="w", padx=(0, 12))
        tk.Label(ar, text="Batch", bg=CONTENT, fg=MUTED, font=(UI_FONT, 11)).grid(row=0, column=2, sticky="w")
        self.batch_var = tk.StringVar(value=str(max(1, min(app.cfg.batch_count, 4))))
        ttk.Combobox(ar, textvariable=self.batch_var, values=["1", "2", "3", "4"], state="readonly", width=6).grid(
            row=1, column=2, sticky="w",
        )

        if mode in ("img2img", "inpaint", "canvas", "upscale", "outpaint") or self.is_video:
            row = tk.Frame(left, bg=CONTENT)
            ...
            self.img_lbl = tk.Label(row, text="No image selected", bg=CONTENT, fg=MUTED, font=(UI_FONT, 12))
            self.img_lbl.pack(side="left", padx=12)
            if mode == "inpaint":
                tk.Button(
                    row, text="Load mask…", command=self._pick_mask,
                    bg=BLUE, fg="#FFFFFF", activebackground=BLUE_HOVER, activeforeground="#FFFFFF",
                    relief="flat", font=("Arial", 12), padx=12, pady=6,
                ).pack(side="left", padx=(8, 0))
                self.mask_lbl = tk.Label(row, text="No mask", bg=CONTENT, fg=MUTED, font=(UI_FONT, 12))
            ...
            if self.is_video:
                ...
                self.vid_lbl = tk.Label(row, text="No clip", bg=CONTENT, fg=MUTED, font=(UI_FONT, 12))
                self.vid_lbl.pack(side="left", padx=12)

        btns = tk.Frame(left, bg=CONTENT)
        ...
        self.status = tk.Label(left, text="Ready", bg=CONTENT, fg=MUTED, font=(UI_FONT, 12))
        self.status.pack(anchor="w", pady=(14, 0))

        # DiffusionBee TwoColAppletLayout.right_workpace + .image_area engine stage
        right = tk.Frame(
            body, bg=ENGINE,
            highlightbackground=ENGINE_BORDER if ENGINE_BORDER != "#00000000" else ENGINE,
            highlightthickness=1 if DARK_MODE else 0,
        )
        right.pack(side="right", fill="both", expand=True, padx=(0, 0))
        eng_head = tk.Frame(right, bg=ENGINE)
        eng_head.pack(fill="x", padx=16, pady=(14, 4))
        tk.Label(
            eng_head, text="Engine", bg=ENGINE, fg=TEXT,
            font=(UI_FONT, 14, "bold"),
        ).pack(side="left")
        tk.Label(
            eng_head, text="Generation stage",
            bg=ENGINE, fg=MUTED, font=(UI_FONT, 11),
        ).pack(side="left", padx=(10, 0))

        stage = tk.Frame(right, bg=ENGINE)
        stage.pack(expand=True, fill="both", padx=16, pady=8)
        stage_inner = tk.Frame(
            stage, bg=ENGINE,
            highlightbackground=("#D0D0D0" if not DARK_MODE else ENGINE_BORDER),
            highlightthickness=1,
        )
        stage_inner.pack(expand=True, fill="both")
        self.preview = tk.Label(
            stage_inner, text="Waiting for generation…", bg=ENGINE, fg=MUTED,
            font=(UI_FONT, 13), wraplength=420, justify="center",
        )
        self.preview.pack(expand=True, fill="both", padx=12, pady=12)

        dl = tk.Frame(right, bg=ENGINE)
        dl.pack(fill="x", padx=16, pady=(0, 14))
        self.save_desktop_btn = tk.Button(
            ...
            relief="flat", font=(UI_FONT, 12, "bold"), padx=10, pady=7, state="disabled",
        )
        ...
        self.bind("<Destroy>", self._on_destroy_workspace)

    def _on_destroy_workspace(self, _event=None):
        if _event is not None and _event.widget is not self:
            return
        try:
            self.app.engine.off_job(self._on_job)
        except Exception:
            pass
        self._watching.clear()

    def _on_model_picked(self, _event=None):
        choice = self.model_var.get()
        if is_model_separator(choice):
            prev = getattr(self, "_last_real_model", None)
            flat = [m for m in self.model_menu.cget("values") if not is_model_separator(m)]
            if prev in flat:
                self.model_var.set(prev)
            elif flat:
                self.model_var.set(flat[0])
            return
        self._last_real_model = choice
        meta = flux_meta(choice)
        if hasattr(self, "model_hint"):
            fam = FAMILY_TITLES.get(meta.get("family", ""), meta.get("family", ""))
            hint = meta.get("label") or fam
            self.model_hint.configure(text=f"{fam} · {hint}" if fam and hint != fam else hint)

    def _toggle_advanced(self):
        open_ = not self._adv_open.get()
        self._adv_open.set(open_)
        self._adv_toggle_lbl.configure(text=("▾ Advanced options" if open_ else "▸ Advanced options"))
        if open_:
            self._adv_frame.pack(fill="x", pady=(0, 8), after=self._adv_head)
        else:
    ...
    def _pick_mask(self):
        path = filedialog.askopenfilename(
            title="Select inpaint mask (white = edit region)",
            filetypes=[("Images", "*.png *.jpg *.jpeg *.webp *.gif"), ("All", "*.*")],
        )
        if path:
            self.mask_image_path = path
            if hasattr(self, "mask_lbl"):
                self.mask_lbl.configure(text=Path(path).name, fg=TEXT)
    ...
    def _as_mask_payload(self) -> Optional[str]:
        if not self.mask_image_path:
            return None
        raw = Path(self.mask_image_path).read_bytes()
    ...
    def _generate(self):
        ...
        if self.mode != "llm" and not self.app.engine.flux.ready():
            ...
            return
        if self.mode in ("img2img", "inpaint", "canvas", "upscale", "outpaint") and not self.input_image_path:
            messagebox.showwarning(APP_NAME, "Load an input image for this mode.")
            return
        if (
            self.mode in ("img2img", "inpaint", "canvas", "upscale", "outpaint")
            and not self.app.engine.flux.api_key
        ):
            messagebox.showwarning(
                APP_NAME,
                "Image edit modes need a BFL API key.\nFree mode only supports text → image.",
            )
            self.app._select_tab("settings")
            return
        ...
        if self.is_video and vmode == "v2v" and not self.start_video_path:
            ...
            return
        if self.is_video and vmode in ("i2v", "v2v") and not self.app.engine.flux.api_key:
            messagebox.showwarning(
                APP_NAME,
                "Image/video-to-video needs a BFL API key.\nFree mode only makes a still from text.",
            )
            self.app._select_tab("settings")
            return
        ...
        try:
            steps_override = int(self.steps_var.get().strip())
        except Exception:
            steps_override = None
        try:
            guidance_override = float(self.guidance_var.get().strip())
        except Exception:
            guidance_override = None
        try:
            batch = max(1, min(int(self.batch_var.get()), 4))
        except Exception:
            batch = 1
        upscale_factor = 2
        if hasattr(self, "upscale_var"):
            try:
                upscale_factor = int(str(self.upscale_var.get()).replace("×", "").strip())
            except Exception:
                upscale_factor = 2

        model = resolve_flux_model_choice(
            self.model_var.get(), "video" if self.is_video else "image"
        )
        ...
        for i in range(batch):
            seed = base_seed if base_seed >= 0 else -1
            if batch > 1 and base_seed >= 0:
                seed = base_seed + i
            job_id = uuid.uuid4().hex[:12]
            # Register before enqueue so worker emits aren't dropped (race).
            self._watching.add(job_id)
            last_job = self.app.engine.enqueue(
                job_id=job_id,
                mode=self.mode,
                prompt=prompt,
                model=model,
                ...
                mask_image=self._as_mask_payload() if self.mode == "inpaint" else None,
        ...
        if last_job:
            label = f"Starting {batch}…" if batch > 1 else "Starting…"
            self.status.configure(text=label, fg=WARN)
    ...
    def _enhance_only(self):
        ...
        def work():
            try:
                self.app.schedule_ui(lambda: self.status.configure(text="Enhancing…", fg=WARN) if self.winfo_exists() else None)
                ...
                self.app.schedule_ui(apply)
            except Exception as e:
                err = str(e)
                self.app.schedule_ui(
                    lambda: self.status.configure(text=err, fg=ERR) if self.winfo_exists() else None
                )

        threading.Thread(target=work, daemon=True).start()

    def _on_job(self, job: GenJob):
        if job.id not in self._watching:
            return
        # Snapshot — job object is mutated on the worker thread.
        snap = (job.id, job.status, job.message or "", job.error or "", job.result_path or "")

        def ui(s=snap):
            if not self.winfo_exists():
                return
            jid, status, message, error, result_path = s
            if status == "queued":
                self.status.configure(text="Starting…", fg=WARN)
                return
            label = {"running": "Running", "done": "Done", "error": "Error"}.get(status, status)
            color = {"running": WARN, "done": OK, "error": ERR}.get(status, MUTED)
            detail = message or error or ""
            self.status.configure(text=f"{label}: {detail}" if detail else label, fg=color)
            if status == "done" and result_path:
                self._show_result(result_path)
                self.app.schedule_ui(self.app.refresh_history_views)
            if status in ("done", "error"):
                self._watching.discard(jid)
                if not self._watching:
                    self.gen_btn.configure(state="normal")

        # NEVER call tk.after from the worker thread on macOS/Tk9 — it silently drops.
        self.app.schedule_ui(ui)
    ...
    def _show_result(self, path: str):
        ...
        if p.suffix.lower() == ".txt":
            ...
            self.preview.configure(text=text, image="", compound="center", bg=ENGINE)
            self._photo = None
            return
        if p.suffix.lower() in (".mp4", ".mov", ".webm"):
            ...
            return
        try:
            img = load_preview_photo(str(p))
            if img is not None:
                self._photo = img
                self.preview.configure(image=img, text="", bg=ENGINE)
                return
        except Exception:
            pass
        try:
...
class App(tk.Tk):
    def __init__(self):
        prepare_macos_tk()
        super().__init__()
        _sanitize_tk_scaling(self)
        self.title(APP_TITLE)
        # DiffusionBee default ~770×550
        self.geometry("770x550")
        self.minsize(770, 550)
        ...
        self._tool_windows: dict[str, ToolWorkspace] = {}
        # Thread-safe UI mailbox — tk.after from worker threads is unreliable on macOS/Tk9.
        self._ui_q: queue.SimpleQueue = queue.SimpleQueue()
        self._ui_pump_after: Optional[str] = None
        ...
        self._splash_after: Optional[str] = None
        self.after(50, self._show_splash)
        self._ui_pump_after = self.after(16, self._pump_ui_queue)
    ...
    def _on_close(self):
        if getattr(self, "_splash_after", None):
            try:
                self.after_cancel(self._splash_after)
            except Exception:
                pass
            self._splash_after = None
        if getattr(self, "_ui_pump_after", None):
            try:
        ...
        try:
            self.engine.off_job(self._on_engine_job)
        except Exception:
            pass
    ...
    def _rebuild_shell(self):
        ...
        for mode, win in list(self._tool_windows.items()):
            try:
                win.destroy()
            except Exception:
                pass
            self._tool_windows.pop(mode, None)
    ...
    def _show_splash(self):
        ...
        tk.Label(
            wrap, text="1.0", bg=CONTENT, fg=MUTED,
            font=(UI_FONT, 18),
        ).pack(pady=(4, 0))
        tk.Label(
            wrap, text="DiffusionBee-compatible · 100% local when keyed",
            bg=CONTENT, fg=MUTED,
            font=(UI_FONT, 12),
        ).pack(pady=(18, 0))
        ...
        def fade():
            self._splash_after = None
            try:
                if self.winfo_exists() and splash.winfo_exists():
                    splash.destroy()
            except Exception:
                pass
        self._splash_after = self.after(1400, fade)
    ...
    def _build_shell(self):
        ...
        canvas.bind("<Enter>", lambda e: canvas.bind("<MouseWheel>", self._on_mousewheel))
    ...
    def _on_mousewheel(self, event):
        ...
        if _sys.platform == "darwin":
            ...
            if steps == 0 and getattr(event, "num", None) == 4:
                steps = -1
            elif steps == 0 and getattr(event, "num", None) == 5:
                steps = 1
        if steps:
    ...
    def _open_overflow_menu(self):
        ...
        m.add_separator()
        m.add_command(label="About OpenPhoto 1.0", command=lambda: messagebox.showinfo(
    ...
    def _select_tab(self, key):
        ...
        if key in ("txt2img", "img2img", "inpaint", "outpaint", "upscale", "txt2vid", "llm"):
            titles = {
                "txt2img": "Text to image",
                "img2img": "Image to image",
                "inpaint": "Inpainting",
    ...
    def _build_settings_page(self, parent):
        ...
        row("Flex guidance (CFG)", "guidance", width=12)
        row("Default negative prompt", "negative_prompt", width=48)
        row("Default batch count", "batch_count", width=12)
        row("Default upscale factor", "upscale_factor", width=12)
        row("Safety tolerance 0–6", "safety_tolerance", width=12)
        tk.Label(
            form,
            text="Output format: PNG (jpeg off — DiffusionBee-style, all Flux models)",
    ...
    def _save_settings(self):
        for key, var in self._settings_vars.items():
            ...
            if key in (
                ...
                "video_duration", "safety_tolerance", "batch_count", "upscale_factor",
            ):
                try:
                    val = int(val)
                except Exception:
    ...
    def open_tool(self, title, mode="txt2img"):
        ...
        if mode == "training":
            ...
            return
        existing = self._tool_windows.get(mode)
        if existing is not None and existing.winfo_exists():
            existing.lift()
            existing.focus_force()
            return
        win = ToolWorkspace(self, title, mode)
        self._tool_windows[mode] = win

        def _clear(_event=None, m=mode, w=win):
            if _event is not None and getattr(_event, "widget", None) is not w:
                return
            if self._tool_windows.get(m) is w:
                self._tool_windows.pop(m, None)
    ...
    def refresh_history_views(self):
        # Render in-memory snapshot only — never reload from disk here
        # (avoids racing the worker thread's history.add/save).
        items = self.history.snapshot(200)
        for key in ("history",):
    ...
    def _on_engine_job(self, job: GenJob):
        status = job.status
        self.schedule_ui(lambda: self.refresh_history_views() if self.winfo_exists() and status == "done" else None)
...
def main() -> None:
    ...
    try:
        ...
        print(
            "OpenPhoto could not start the Tk UI on this macOS / Python build.\n"
            f"{exc}\n"
            "Tip: use the Mojo/uv python3 that ships Tk 9, or set OPENPHOTO_SKIP_TK_FIX=1 only for debugging.",
            file=_sys.stderr,
        )
